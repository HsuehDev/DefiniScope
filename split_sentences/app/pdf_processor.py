    ...
    for i, token in enumerate(doc[:-1]):
        ...
        if token.text in strong_end_marks and not is_abbreviation:
            if i + 1 < len(doc):
                ...
                elif next_token.text == "(" and i + 5 < len(doc) and \
                     any(doc[i+j].text == ")" for j in range(2, 6)):
                    continue
...
class PDFProcessor:
    ...
    def _handle_special_cases(self, sentences: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        處理特殊情況，進一步優化句子分割
        ...
        while i < len(sentences):
            ...
            # 處理連續片段
            if i + 1 < len(sentences) and current["page"] == sentences[i+1]["page"]:
                next_sent = sentences[i+1]["sentence"]
                current_sent = current["sentence"]
                
                # 情況1: 引用模式 - "(作者, 年份)" 後面跟著另一句話
                if re.search(r'\([A-Za-z]+(\s+et al\.)?,\s+\d{4}\)(\.|\,)?$', current_sent):
                    merged_sentence = f"{current_sent} {next_sent}"
                    result.append({"sentence": merged_sentence, "page": current["page"]})
                    merged = True
                    i += 2
                    
                # 情況2: 縮寫結尾
                elif any(current_sent.endswith(abbr) for abbr in ["e.g.", "i.e.", "etc.", "vs.", "cf."]):
                    merged_sentence = f"{current_sent} {next_sent}"
                    result.append({"sentence": merged_sentence, "page": current["page"]})
                    merged = True
                    i += 2
                ...
                # 情況4: 句子過短且不以標點結尾
                elif len(current_sent) < 30 and not re.search(r'[\.\!\?\。\？\！]\s*$', current_sent):
                    merged_sentence = f"{current_sent} {next_sent}"
                    result.append({"sentence": merged_sentence, "page": current["page"]})
                    merged = True
                    i += 2
            
            # 如果沒有合併，添加當前句子並移動到下一個
            if not merged:
    ...
    def split_into_sentences(self, pages: List[Dict[str, Any]], status: ProcessingStatus = None) -> List[Dict[str, Any]]:
        ...
        # 第一階段：合併片段並過濾無效句子
        merged_sentences = self.combine_sentence_fragments(sentences)
        
        # 第二階段：處理特殊情況
        processed_sentences = self._handle_special_cases(merged_sentences)
        
        # 最後階段：進行最終的過濾
        return [s for s in processed_sentences if self._is_valid_sentence(s["sentence"])]
